...
| | `status` (`status_fields` / `parse_status`, published by `StatusWriter`) | a `PlayerStatus`: the item on screen, the playhead, paused, locked, whether the item is a picture — and after those six lines, whatever that player adds of its own |
